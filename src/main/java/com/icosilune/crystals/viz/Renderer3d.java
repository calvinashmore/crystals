...
//    for(int i=0;i<points.size();i++) {
      ...
      Point3d difference = dot.getPoint().subtract(c);
      double dd = difference.norm();
...
//      g.setColor(new Color(1, progress, blue));
      ...
      AffineTransform pop = g.getTransform();
      
      // these numbers are arbitrary
      dd = dd*dd/10;
      g.translate(getWidth()*x, getHeight()*y);
      g.scale(10.0/dd, 10.0/dd);
      g.scale(dot.getSize(), dot.getSize());
      
      dot.getRenderer().render(g);
